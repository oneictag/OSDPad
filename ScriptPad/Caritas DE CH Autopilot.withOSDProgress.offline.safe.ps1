...
# --- Check WPF support (WinPE often lacks PresentationFramework) -----
$SupportsWPF = $true
try { Add-Type -AssemblyName PresentationFramework -ErrorAction Stop } catch { $SupportsWPF = $false }
if (-not $SupportsWPF) {
    Write-Host -ForegroundColor Yellow "[!] WPF nicht verfügbar (PresentationFramework). OSDProgress wird übersprungen."
} else {
    Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('yyyy-MM-dd-HHmmss'))  [PreOS] Starte OSDProgress..."
    # Start OSDProgress in a separate job with timeout so we never hang
    $Template = 'C:\Service\OSDCaritas\Config\Caritas.OSDProgress.psd1'
    $job = Start-Job -ScriptBlock {
        param($tpl)
        Import-Module OSDProgress -Force
        if (Test-Path $tpl) { Invoke-OSDProgress -Style Win10 -TemplateFile $tpl }
        else { Invoke-OSDProgress -Style Win10 }
        # Signal started
        '[OSDProgressStarted]'
    ...
    if (Wait-Job $job -Timeout 10) {
        ...
        if ($out -match 'OSDProgressStarted') {
            ...
            Write-Host -ForegroundColor Yellow "[!] OSDProgress-Job liefert kein Startsignal – fahre ohne Overlay fort."
        }
    } else {
