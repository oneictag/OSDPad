...
# --- Caritas Assets holen (robust) -----------------------------------
$ProgressPreference = 'SilentlyContinue'
New-Item 'C:\Service\OSDCaritas\Config' -ItemType Directory -Force | Out-Null

Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('yyyy-MM-dd-HHmmss'))  [PreOS] Netztest raw.githubusercontent.com:443"
try {
    $ok = Test-NetConnection raw.githubusercontent.com -Port 443 -InformationLevel Quiet
    Write-Host -ForegroundColor $(if($ok){'Green'}else{'Yellow'}) ("[{0}] Connectivity" -f ($(if($ok){'OK'}else{'FAIL'})))
} catch { Write-Host -ForegroundColor Yellow "[!] Test-NetConnection nicht verfügbar in WinPE" }

function Get-Asset($Url, $Dest) {
    $name = [IO.Path]::GetFileName($Dest)
    if (Test-Path $Dest) {
        Write-Host -ForegroundColor Green "[=] $name vorhanden, verwende Cache"
        return $true
    }
    Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('yyyy-MM-dd-HHmmss'))  [PreOS] Lade $name ..."
    try {
        Invoke-WebRequest -Uri $Url -OutFile $Dest -UseBasicParsing `
            -Headers @{ 'User-Agent'='OSDCaritas' } -TimeoutSec 20 -ErrorAction Stop
        Write-Host -ForegroundColor Green "[+] $name heruntergeladen"
        return $true
    } catch {
        Write-Host -ForegroundColor Yellow "[!] Download $name fehlgeschlagen: $($_.Exception.Message)"
        return $false
    }
}

$LogoOK = Get-Asset 'https://raw.githubusercontent.com/oneictag/OSDPad/main/Caritas_Schweiz_Logo_rot-weiss.png' `
                    'C:\Service\OSDCaritas\Config\Caritas_Schweiz_Logo_weiss.png'
$TplOK  = Get-Asset 'https://raw.githubusercontent.com/oneictag/OSDPad/main/Caritas.OSDProgress.psd1' `
                    'C:\Service\OSDCaritas\Config\Caritas.OSDProgress.psd1'

$TemplatePath = 'C:\Service\OSDCaritas\Config\Caritas.OSDProgress.psd1'
if (-not (Test-Path $TemplatePath)) {
    Write-Host -ForegroundColor Yellow "[!] Template nicht verfügbar – starte OSDProgress ohne Template."
}
# ---------------------------------------------------------------------
